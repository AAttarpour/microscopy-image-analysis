...
- eccentricity_thr: thr for removing small object based on eccentricity (between 0-1; circles have eccentricity of 0)
- orientation_thr: thr for removing small object based on orientation (degree)
...
my_parser.add_argument('-t','--remove_small_obj_thr', help='thr (number of voxels) for removing small object', required=False, default=200, type=int)
...
my_parser.add_argument('-d','--dilate_distance_transform_flag', help='whether to dilate distance transform', required=False, default=True, type=bool)
my_parser.add_argument('-e','--eccentricity_thr', help='thr for removing small object based on eccentricity (between 0-1; circles have eccentricity of 0)', required=False, default=0.5, type=float)
my_parser.add_argument('-or','--orientation_thr', help='thr for removing small object based on orientation (degree)', required=False, default=70, type=int)

# -------------------------------------------------------
# filter function
# -------------------------------------------------------
def filter_objects(binary_map, min_size=100, eccentricity_threshold=0.5, orientation_threshold=70):

    # binarize the input_map
    # binary_map_filtered = remove_small_objects(binary_map, min_size=min_size, connectivity=3)

    # run connected component analysis
    # labeled_binary_map, _ = scp.label(binary_map.astype(np.uint8), structure=np.ones((3,3,3)))
    ...
    labeled_binary_map_2d_filtering = np.zeros_like(binary_map, dtype=bool)

    for z in range(binary_map.shape[0]):  # Iterate over z-axis

         # run 2D connected component analysis
        # slice_labels = labeled_binary_map[z]
        slice_labels, _ = scp.label(binary_map.astype(np.uint8)[z, ...], structure=np.ones((3,3)))
        
        # Use regionprops_table for slice
        props_table = measure.regionprops_table(slice_labels, 
                                    properties=["label", "eccentricity", "orientation"])
    ...
    labeled_binary_map_3d_filtering, _ = scp.label(labeled_binary_map_2d_filtering, structure=np.ones((3,3,3)))

    # Use regionprops_table for 3D
    props_table_3d = measure.regionprops_table(labeled_binary_map_3d_filtering, 
...
def aa_skeletonize(idx,
    ...
    for z in range(combined_distance.shape[0]):  # Iterate over slices
        ...
        if local_maxima.size > 0: combined_distance_ridge[z][tuple(local_maxima.T)] = True
    ...
    # Binarize the output
    # binarized_thr = 0.1
    # combined_skeleton_bin = (combined_distance_ridge > binarized_thr)

    # print(combined_skeleton_bin.shape)
    # save_tiff(combined_distance_ridge, 'skeletonized_binarized_' + os.path.split(file_names_paths[idx])[1], out_dir, np.bool_)
    # print(f"combined_skeleton_binarized is saved / shape: {combined_skeleton_bin.shape}")

    # # filter small objects
    combined_distance_ridge_filtered = filter_objects(combined_distance_ridge,
                                                      min_size=min_size_thr, 
                                                      eccentricity_threshold=eccentricity_thr, 
                                                      orientation_threshold=orientation_thr)
    
    save_tiff(combined_distance_ridge_filtered, os.path.split(file_names_paths[idx])[1], out_dir, np.bool_)
    print(f"skeleton_filtered is saved / shape for {os.path.split(file_names_paths[idx])[1]}")
# -------------------------------------------------------
# main function
# -------------------------------------------------------
...
def main():
    ...
    eccentricity_thr = args['eccentricity_thr']
    orientation_thr = args['orientation_thr']
    ...
    with parallel_config(backend="threading", n_jobs=ncpus):
        ...
        Parallel()(delayed(aa_skeletonize)(
